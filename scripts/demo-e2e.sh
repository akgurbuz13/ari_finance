...
WALLET_ADDR=$(docker exec ova_v1-postgres-1 psql -U ova_user -d ova_platform -t -c "
...
OUTBOX_EVENTS=$(docker exec ova_v1-postgres-1 psql -U ova_user -d ova_platform -t -c "
...
if [ -n "$WALLET_ADDR" ]; then
    ...
    EU_WALLET=$(docker exec ova_v1-postgres-1 psql -U ova_user -d ova_platform -t -c "
